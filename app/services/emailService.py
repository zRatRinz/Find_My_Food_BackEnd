...
import smtplib
import requests
from app.core.config import SENDER_EMAIL, SENDER_PASSWORD, BREVO_API_KEY, BREVO_SENDER_EMAIL, BREVO_URL
from app.core.exceptions import ExternalServerException

def send_otp_email(receiver_email: str, otp: str):
    try:
        # 1. เตรียมกล่องจดหมายแบบผสม (Multipart)
        message = MIMEMultipart("alternative")
        message["From"] = f"Find My Food <{SENDER_EMAIL}>"
        message["To"] = receiver_email
        message["Subject"] = "[Find My Food] รหัส OTP สำหรับรีเซ็ตรหัสผ่าน"

        text_content = f"""
        สวัสดีครับ,
        
        เราได้รับคำขอให้รีเซ็ตรหัสผ่านสำหรับบัญชี Find My Food ของคุณ 
...
def send_otp_email_brevo(receiver_email: str, otp: str):
    html_template = f"""
    <!DOCTYPE html>
    <html>
    <head>
